# -*- coding: utf-8 -*-
"""
Created on Sun Sep  5 16:35:50 2021
...
def Astar(s, G):
    ...
    pq = []
    # The cost of n be 0 here.
    pq.append([(hv + 0), Node(start,None,hv,0)])
    nodesExpanded = 0
...
#           print(len(visitedStates) - nodesExpanded , "Nodes not expanded.")
            print(len(visitedStates), "Nodes in total including the start node.")
            n = len(visitedStates)
            return d,nodesExpanded,n
        boardState.up(state[1],copy.deepcopy(boardState), pq, visitedStates, GOAL, "A")
        boardState.right(state[1],copy.deepcopy(boardState), pq, visitedStates, GOAL, "A")
        boardState.down(state[1],copy.deepcopy(boardState), pq, visitedStates, GOAL, "A")
        boardState.left(state[1],copy.deepcopy(boardState), pq, visitedStates, GOAL, "A")
        nodesExpanded += 1
    return 0,0,0